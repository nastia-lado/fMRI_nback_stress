...
for run in runs: 
    ...
    for p, (i, atlas) in enumerate(nback):
        correlation_matrices_wei_z = np.load(f'{top_dir}/func_connect/04-correlation_matrices/static/LB_{nback[p,1]}_static_correlation_matrices_wei_z_{run}.npy')
        correl_matrices_wei_z.append(correlation_matrices_wei_z)
    
    #correlation matrices atlas-based non-weighted
    # correl_matrices_z = []
    # for p, (i, atlas) in enumerate(nback):
    #     correlation_matrices_z = np.load(f'{top_dir}/func_connect/04-correlation_matrices/static/LB_{nback[p,1]}_static_correlation_matrices_z_{run}.npy')
    ...
    with open(f'{out_dir}static/all_ttests_pre-post_matrices_wei_{run}.txt', 'w') as f:
    ...
    with open(f'{out_dir}static/ttests_post_sport-control_matrices_wei_{run}.txt', 'w') as f:
        for item in list_strings:
            f.write("%s\n" % item)
    ...
    with open(f'{out_dir}static/ttests_post_sport-meditation_matrices_wei_{run}.txt', 'w') as f:
        for item in list_strings:
            f.write("%s\n" % item)
    #%post med/control
    for i, (j, atlas) in enumerate(nback):
    ...
    with open(f'{out_dir}static/ttests_post_meditation-control_matrices_wei_{run}.txt', 'w') as f:
        for item in list_strings:
            f.write("%s\n" % item)
