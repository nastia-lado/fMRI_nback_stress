...
from nilearn import datasets, input_data, signal, image  
...
ho_cort_masker = NiftiLabelsMasker(labels_img=ho_cort_filename, standardize=True, resampling_target = 'labels',
                           memory='nilearn_cache', verbose=5)
ho_sub_masker = NiftiLabelsMasker(labels_img=ho_sub_filename, standardize=True, resampling_target = 'labels',
                           memory='nilearn_cache', verbose=5)

#Talairach (72)
talairach = datasets.fetch_atlas_talairach('ba')
talairach_filename = talairach.maps
talairach_masker = NiftiLabelsMasker(labels_img=talairach_filename, standardize=True, resampling_target = 'labels',
                           memory='nilearn_cache', verbose=5)
        ...
        for run in runs:
            ...
            for i, sub in enumerate(subs):
                ...
                for j, ses in enumerate(sess):
                    ...
                    # Filtering data
                    #filtering is done at a previous step
                    #better to keep all the filtering in one func to ensure the correct order of steps
                    #specified by Linquist paper
