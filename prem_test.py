...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar  4 12:38:13 2021
...
## Modified by alana to return permutation scores, along with pvals
...
def permutation_test(x, y, func='x_mean != y_mean', method='exact',
    ...
    if method == 'exact':
        ...
        for i in range(num_rounds):
            ...
            test_stat = func(combined[:m], combined[m:])
            scores.append(test_stat)
            if func(combined[:m], combined[m:]) >= reference_stat:
                more_extreme += 1.

    pval = more_extreme / num_rounds

    return pval, scores, reference_stat