...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep  6 15:18:07 2021
...
for k, contrast in enumerate(contr_fl_interest):
    ...
    s.savefig(f'{save_path}/figures/permut/allgroups_{contrast}_thresh_zmap_permut.png')
    ...
    for section in sections:
        s = plot_stat_map(neg_log_pvals_permuted_ols_unmasked, 
                      ...
                      threshold=threshold,
                      title=title)
        s.savefig(f'{save_path}/figures/permut/allgroups_{contrast}_thresh_zmap_permut_{section}.png')
    plotting.show()
    ...
    table = get_clusters_table(neg_log_pvals_permuted_ols_unmasked, stat_threshold=threshold,
                               cluster_threshold=1, min_distance=0)
    if not os.path.exists(save_path):
        os.makedirs(save_path)   
    table.to_csv(f'{save_path}/tables/permut/allgroups_clusters_table_{contrast}_neg_log_pvals_permuted_ols.csv')
...
for k, contrast in enumerate(contr_fl_interest):
    ...
    s = plotting.plot_stat_map(neg_log_pvals_permuted_ols_unmasked, colorbar=True, 
                               vmax=3, threshold=threshold, title=title)
    plotting.show()
...
def create_clusters_csv(top_dir, file_name, comp_type):
    ...
    print(f'clusters csv if created for {file_name[:-2]} {comp_type}')
    return
