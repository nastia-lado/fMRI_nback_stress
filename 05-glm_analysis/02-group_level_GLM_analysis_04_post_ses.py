...
for k, contrast in enumerate(contr_fl_interest):
    ...
    for i, sub in enumerate(subs):
        zmaps.append(f'{top_dir}/GLM/FLA/voxel/grey_matter_mask/{sub}_ses-3_space-MNI152NLin2009cAsym_{contrast}.nii.gz')
    ...
    for contrast_id, contrast_val in enumerate(contrasts_sl):
        ...
        s = plotting.plot_stat_map(
            ...
            title=title)
        s.savefig(f'{out_dir}/voxel/grey_matter_mask/post/figures/fpr/{contrast_val}_post_{contrast}_thresh_zmap_fpr<0.005.png')
        ...
        save_dir = f'{out_dir}/voxel/grey_matter_mask/post/tables/fpr/{contrast_val}_post_clusters_table_{contrast}_fpr.csv'
        ...
        for section in sections:
            s = plot_stat_map(thresholded_map2, 
                          ...
                          black_bg=False,
                          title=title)
            s.savefig(f'{out_dir}/voxel/grey_matter_mask/post/figures/fdr/{contrast_val}_post_{contrast}_thresh_zmap_fdr<0.05_{section}.png')
        plotting.show()
        ...
        save_dir = f'{out_dir}/voxel/grey_matter_mask/post/tables/fdr/{contrast_val}_post_clusters_table_{contrast}_fdr.csv'
        table.to_csv(save_dir)
        ...
        for section in sections:
            s = plot_stat_map(thresholded_map3, 
                          ...
                          black_bg=False,
                          title=title)
            s.savefig(f'{out_dir}/voxel/grey_matter_mask/post/figures/bonferroni/{contrast_val}_post_{contrast}_thresh_zmap_bonferroni_{section}.png')
        plotting.show()
        ...
        table = get_clusters_table(z_map, stat_threshold=threshold3,
                                   cluster_threshold=3, min_distance=2.0)
        save_dir = f'{out_dir}/voxel/grey_matter_mask/post/tables/bonferroni/{contrast_val}_post_clusters_table_{contrast}_bonferroni.csv'
        table.to_csv(save_dir)
...
for k, contrast in enumerate(contr_fl_interest):
    ...
    for i, sub in enumerate(subs_sport):
        zmaps_sport.append(f'{top_dir}/GLM/FLA/voxel/grey_matter_mask/{sub}_ses-3_space-MNI152NLin2009cAsym_{contrast}.nii.gz')
    
    zmaps_med = []
    for i, sub in enumerate(subs_med):
        zmaps_med.append(f'{top_dir}/GLM/FLA/voxel/grey_matter_mask/{sub}_ses-3_space-MNI152NLin2009cAsym_{contrast}.nii.gz')
    ...
    s = plotting.plot_stat_map(
        thresholded_map1, threshold=threshold1, colorbar=True,
        title=title)
    s.savefig(f'{out_dir}/voxel/grey_matter_mask/post_1vs1/figures/fpr/{contrast_val}_post_{contrast}_thresh_zmap_fpr<0.005.png')
    for section in sections:
        s = plot_stat_map(thresholded_map1, 
                      threshold=threshold1,
                      display_mode=section, 
                      ...
                      black_bg=False,
                      title=title)
        s.savefig(f'{out_dir}/voxel/grey_matter_mask/post_1vs1/figures/fpr/{contrast_val}_post_{contrast}_thresh_zmap_fpr<0.005_{section}.png')
    plotting.show()

    thresh_zmap = image.threshold_img(thresholded_map1, threshold=threshold1)
    nib.save(thresh_zmap, f'{out_dir}/voxel/grey_matter_mask/post_1vs1/{contrast_val}_post_fpr_thresh-001_{contrast}.nii.gz')
    
    table = get_clusters_table(z_map, stat_threshold=threshold1,
                               cluster_threshold=3, min_distance=1.0)
    save_dir = f'{out_dir}/voxel/grey_matter_mask/post_1vs1/tables/fpr/{contrast_val}_post_clusters_table_{contrast}_fpr.csv'
    table.to_csv(save_dir)
    ...
    for section in sections:
        s = plot_stat_map(thresholded_map2, 
                      ...
                      black_bg=False,
                      title=title)
        s.savefig(f'{out_dir}/voxel/grey_matter_mask/post_1vs1/figures/fdr/{contrast_val}_post_{contrast}_thresh_zmap_fdr<0.05_{section}.png')
    plotting.show()
    
    thresh_zmap = image.threshold_img(thresholded_map2, threshold=threshold2)
    nib.save(thresh_zmap, f'{out_dir}/voxel/grey_matter_mask/post_1vs1/{contrast_val}_post_fdr_thresh-05_{contrast}.nii.gz')
    
    table = get_clusters_table(z_map, stat_threshold=threshold2,
                               cluster_threshold=3, min_distance=2.0)
    save_dir = f'{out_dir}/voxel/grey_matter_mask/post_1vs1/tables/fdr/{contrast_val}_post_clusters_table_{contrast}_fdr.csv'
    table.to_csv(save_dir)
    ...
    for section in sections:
        s = plot_stat_map(thresholded_map3, 
                      ...
                      black_bg=False,
                      title=title)
        s.savefig(f'{out_dir}/voxel/grey_matter_mask/post_1vs1/figures/bonferroni/{contrast_val}_post_{contrast}_thresh_zmap_bonferroni_{section}.png')
    plotting.show()
    
    thresh_zmap = image.threshold_img(thresholded_map3, threshold=threshold3)
    nib.save(thresh_zmap, f'{out_dir}/voxel/grey_matter_mask/post_1vs1/{contrast_val}_post_bonferroni_thresh-05_{contrast}.nii.gz')
    
    table = get_clusters_table(z_map, stat_threshold=threshold3,
                               cluster_threshold=3, min_distance=2.0)
    save_dir = f'{out_dir}/voxel/grey_matter_mask/post_1vs1/tables/bonferroni/{contrast_val}_post_clusters_table_{contrast}_bonferroni.csv'
    table.to_csv(save_dir)
